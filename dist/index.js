...
// node_modules/@inlang/core/dist/utilities/result.js
var _Result = class {
  unwrap(ok, err) {
...
(function(Result2) {
  ...
  function all(obj) {
    ...
    for (let i = 0; i < keys2.length; i++) {
      const item = obj[keys2[i]];
      if (item.isErr) {
        return item;
      }
      res[keys2[i]] = item.value;
    }
...
// node_modules/@inlang/core/dist/query/index.js
...
function create(resource, args) {
  ...
  if (get(copy2, { id: args.message.id.name })) {
    return Result.err(Error(`Message ${args.message.id.name} already exists in resource ${resource.languageTag.name}.`));
  }
  copy2.body.push(args.message);
  return Result.ok(copy2);
}
...
function update(resource, args) {
  ...
  for (const [i, message] of resource.body.entries()) {
    if (message.id.name === args.id) {
      copy2.body[i] = args.with;
      return Result.ok(copy2);
    }
  }
  return Result.err(Error(`Message ${args.id} does not exist in resource ${resource.languageTag.name}.`));
}
function _delete(resource, args) {
  const copy2 = JSON.parse(JSON.stringify(resource));
  for (const [i, message] of resource.body.entries()) {
    if (message.id.name === args.id) {
      copy2.body.splice(i, 1);
      return Result.ok(copy2);
    }
  }
  return Result.err(Error(`Message ${args.id} does not exist in resource ${resource.languageTag.name}.`));
...
@inlang/core/dist/query/index.js:
